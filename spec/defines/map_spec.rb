...
describe 'seil::map' do
  ...
  it do
    should contain_seil__exec('seil::map enable_foobar 1').with({
      :command => "set enable_foobar 1", :unless => "set enable_foobar 1"
    })

    should contain_seil__exec('seil::map keycode_foobar 28').with({
  ...
  context 'with identifier set to barfoo and value set to 42' do
    ...
    it do
      should contain_seil__exec('seil::map enable_barfoo 1').with({
        :command => "set enable_barfoo 1", :unless => "set enable_barfoo 1"
      })

      should contain_seil__exec('seil::map keycode_barfoo 42').with({
